...
BoundingBoxGC::BoundingBoxGC() : BoundingBoxGC({COLOR(0xffffffff)}) {}

BoundingBoxGC::BoundingBoxGC(SDL_Color color) : color_(color) {}
...
void BoundingBoxGC::render(Container* c, Uint32 time) {
  ...
  double wh = std::min(c->getHeight(), c->getWidth()) /
              2.0;  // minimum of width an height
  vel = vel * wh / 2.0;
