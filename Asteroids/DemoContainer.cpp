...
DemoContainer::DemoContainer(SDLGame* game)
    : Container(game),
      tennisball_(game->getServiceLocator()->getTextures()->getTexture(
          Resources::TennisBall)),
...
void DemoContainer::reset() {
  setWidth(15);
  setHeight(15);
  setPosition(Vector2D(getGame()->getWindowWidth() / 2 - 6,
                       getGame()->getWindowHeight() / 2 - 6));
