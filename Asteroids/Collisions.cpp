...
bool Collisions::collidesWithRotation(GameObject* o1, GameObject* o2) {
  Vector2D Ac =
      o1->getPosition() + Vector2D(o1->getWidth() / 2.0, o1->getHeight() / 2.0);

  double angleA = o1->getRotation();

  Vector2D Alu =
      Ac +
      Vector2D(-o1->getWidth() / 2.0, -o1->getHeight() / 2.0).rotate(angleA);
  Vector2D Aru =
      Ac +
      Vector2D(o1->getWidth() / 2.0, -o1->getHeight() / 2.0).rotate(angleA);
  Vector2D All =
      ...
      Vector2D(-o1->getWidth() / 2.0, o1->getHeight() / 2.0).rotate(angleA);
  Vector2D Arl =
      Ac + Vector2D(o1->getWidth() / 2.0, o1->getHeight() / 2.0).rotate(angleA);

  double angleB = o2->getRotation();

  Vector2D Bc =
      o2->getPosition() + Vector2D(o2->getWidth() / 2.0, o2->getHeight() / 2.0);

  Vector2D Blu =
      Bc +
      Vector2D(-o2->getWidth() / 2.0, -o2->getHeight() / 2.0).rotate(angleB);
  Vector2D Bru =
      Bc +
      Vector2D(o2->getWidth() / 2.0, -o2->getHeight() / 2.0).rotate(angleB);
  Vector2D Bll =
      ...
      Vector2D(-o2->getWidth() / 2.0, o2->getHeight() / 2.0).rotate(angleB);
  Vector2D Brl =
      Bc + Vector2D(o2->getWidth() / 2.0, o2->getHeight() / 2.0).rotate(angleB);
