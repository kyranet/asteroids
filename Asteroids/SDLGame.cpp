...
void SDLGame::initSDL() {
  ...
  // Create window .
  window_ = SDL_CreateWindow(windowTitle_.c_str(), SDL_WINDOWPOS_UNDEFINED,
...
SDL_Window* SDLGame::getWindow() const { return window_; }

SDL_Renderer* SDLGame::getRenderer() const { return renderer_; }

const ServiceLocator* SDLGame::getServiceLocator() const { return &services_; }

int SDLGame::getWindowWidth() const { return width_; }

int SDLGame::getWindowHeight() const { return height_; }

void SDLGame::addObserver(Observer* o) { observers_.push_back(o); }

void SDLGame::send(const void* senderObj, const msg::Message& msg) {
  for (Observer* o : observers_) {
    if (senderObj != o) {
      if (msg.destination_ ==
              msg::Broadcast  // we send to everyone, even to the one from whom
