...
struct Shoot: public msg::Message {
	Shoot(msg::ObjectId sender, msg::ObjectId destination, Vector2D pos, Vector2D dir, int bulletType) :
	Message(msg::FIGHTER_SHOOT, sender, destination), pos_(pos), dir_(dir), bulletType_(bulletType) {
	}
...
struct FighterAsteroidCollision: public msg::Message {
	FighterAsteroidCollision(msg::ObjectId sender, msg::ObjectId destination, GameObject* fighter, Asteroid* asteroid) :
...
struct BulletsInfo: public msg::Message {
	BulletsInfo(msg::ObjectId sender, msg::ObjectId destination, const std::vector<Bullet*>* bullets) :
