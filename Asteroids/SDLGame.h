...
class SDLGame {
 public:
  ...
  const ServiceLocator* getServiceLocator()
      const;  // returns a pointer to the resources
 ...
 private:
  void initSDL();        // initialize SDL (ttf, mixer, image, create window and
                         // renderer, etc)
  void closeSDL();       // close all SDL resources
  void initResources();  // initialize the SDLResources object with the data at
